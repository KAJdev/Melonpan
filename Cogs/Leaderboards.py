...
class Leaderboards(commands.Cog):
    ...
    async def top(self, ctx):
        ...
        for user in top_global:
            ...
            desc += f"`#{amount}` • {user['money']} <:BreadCoin:815842873937100800> • **{user['user_object']}**{' ' if len(user.get('badges', [])) > 0 else ''}{''.join(config.badges[x]['emoji'] for x in user.get('badges', []))}\n"
            amount += 1
